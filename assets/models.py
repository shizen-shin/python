# coding: utf-8
from sqlalchemy import Column, Integer, String, Boolean, DateTime, Date, Float, DECIMAL  #型を宣言。一通り宣言しておく
...
class Data(Base):  #「Data」というDBを作成 (エクセルのファイル名)
    ...
    start = Column(DECIMAL, unique=False)  #型:Integer
    high = Column(DECIMAL, unique=False)
    low = Column(DECIMAL, unique=False)
    end = Column(DECIMAL, unique=False)
    adjusted = Column(DECIMAL, unique=False)
